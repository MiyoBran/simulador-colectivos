...
public class GeneradorPasajeros {
	...
	// private final Map<String, Parada> paradasDisponibles; // Eliminada porque no se usa
	 ...
	 * @param configProperties Las propiedades de configuración (debe incluir "cantidadPasajeros").
	 * @throws IllegalArgumentException si los mapas o propiedades son nulos o están vacíos,
     * o si la propiedad 'cantidadPasajeros' falta, está vacía, o no es un número positivo.
	 */
     ...
     * Cada pasajero es creado y asignado a la cola de espera de su parada de origen.
	 *
	 * @return Una lista con todas las instancias de Pasajero generadas.
     * @throws IllegalStateException si no hay líneas disponibles para generar pasajeros.
	 */
    ...
    private int leerCantidadPasajeros(Properties configProperties) {
        ...
        if (cantidadStr == null || cantidadStr.trim().isEmpty()) {
            throw new IllegalArgumentException("La propiedad 'cantidadPasajeros' no está definida o está vacía en config.properties.");
        }
        try {
            ...
            throw new IllegalArgumentException("La propiedad 'cantidadPasajeros' en config.properties no es un número válido.", e);
        }
    }

    /**
     * Selecciona aleatoriamente una parada de origen y una de destino de una línea dada.
     ...
     * @return un array de Parada de tamaño 2, donde el índice 0 es el origen y el 1 es el destino.
     */
    private Parada[] seleccionarParadasOrigenDestino(Linea linea) {
        ...
        // la de excluir las N últimas paradas que tenías antes, pero la mantenemos simple.
